...
VkPipeline vulkan_pipeline_create(struct vulkan* vulkan, char* vertPath, char* fragPath) {
  ...
  // Shaders
  VkShaderModule vertShaderModule = vulkan_shader_module_create(vulkan, vertPath);
  VkShaderModule fragShaderModule = vulkan_shader_module_create(vulkan, fragPath);
  ...
  if (vkCreateGraphicsPipelines(vulkan->device, VK_NULL_HANDLE, 1, &pipelineInfo, NULL, &pipeline) != VK_SUCCESS) {
...
void vulkan_pipeline_add(struct vulkan* vulkan, VkPipeline pipeline) {
  if (vulkan->pipelineCount == MAX_PIPELINES) {
    ERROR("Too many pipelines!\n");
  }
  vulkan->pipelines[vulkan->pipelineCount++] = pipeline;
}
